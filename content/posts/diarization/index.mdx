...
Speech processing systems can be broadly split into two categories:

+ **Speech Recognition**: Task of detecting and identifying words in the spoken language.
+ **Speaker Recognition**: Task of identifying speakers in a conversation.

Speaker Diarization falls in the second category of speech processing where it is required to identify the speaker along with identification of the boundary/frame of the speech region of a particular speaker.
...
+ Whether the agent was patient enough listeing to the customer or was rushing on the call
...
+ **Clustering** — Finally we cluster the segment wise embedding using a simple **K-Means** to produce diarization results and determine the number of speakers with each speakers time stamps.
